...
export async function getProducts({ category, sort, size, colour, price, onSale, inStock, limit, featured } = {}) {
  await delay();
  ...
  if (price && PRICE_RANGE_MAP[price]) {
    ...
    result = result.filter((p) => p.base_price >= min && p.base_price <= max);
  }
  if (onSale) result = result.filter((p) => p.compare_price && p.compare_price > p.base_price);
